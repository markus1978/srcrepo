setwd("/Users/markus/Documents/Projects/srcrepo-mars/08-data")
import_data <- read.csv(file="importdata.csv", head=TRUE, sep=",")

import_data <- import_data[with(import_data, order(-X2_dbSize)),]
import_data <- head(import_data, 10)

bar_data <- cbind(rbind(import_data$X3_checkoutTimeSum,import_data$X4_refreshTimeSum, import_data$X5_importTimeSum, import_data$X6_writeTimeSum/1000, 0),rbind(0,0,0,0,import_data$X6_traverseTime))
