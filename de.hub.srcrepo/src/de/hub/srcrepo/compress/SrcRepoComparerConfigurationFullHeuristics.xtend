...
abstract class SrcRepoComparerConfigurationFullHeuristics implements ComparerConfiguration {
		...
		return switch original {
			...
			AbstractMethodInvocation: {
				if (original.eClass == revised.eClass) {
					original.method?.name == (revised as AbstractMethodInvocation).method?.name	
				} else {
					false
				}
			}
			TypeAccess: {
				if (original.eClass == revised.eClass) {
					original.id(true) == (revised as TypeAccess).id(false)				
				} else {
					false
				}
			}
			SingleVariableAccess: {
				if (original.eClass == revised.eClass) {
					original.variable?.name == (revised as SingleVariableAccess).variable?.name
				} else {
					false
				}
			}
